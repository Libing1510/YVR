    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
                ...
                half factor = pow(1 - saturate(dot(normalize(inputData.viewDirectionWS), inputData.normalWS)), _Power);

                half Ndot_noSat = dot(mainLight.direction, inputData.normalWS);

                half NdotL_half = saturate(lerp(Ndot_noSat, 1, _DiffuseWrap));
                half NdotL = saturate(Ndot_noSat);

                color.rgb = surfaceData.albedo * NdotL;
                color.rgb += _BaseColor.rgb * _Shininess * (factor) * NdotL_half;
