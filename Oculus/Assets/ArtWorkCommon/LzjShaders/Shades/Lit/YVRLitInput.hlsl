#ifndef YVR_LIT_INPUT_INCLUDED
#define YVR_LIT_INPUT_INCLUDED
...
UNITY_DOTS_INSTANCED_PROP(float, _LightMapIntensity)
...
#define _LightMapIntensity		UNITY_ACCESS_DOTS_INSTANCED_PROP_FROM_MACRO(float, Metadata__LightMapIntensity)
...
#if defined(_RECTANGLELIT_COOKIE_SUPPORT) && !defined(_RECTANGLELIT_COOKIE_OFF)
half	  _Intensity;
float4x4  _RectWorldToLightMatrix;
float3	  _RectLightLocalCenterPos;
float3	  _RectLightLocalSize;
TEXTURE2D(_CookieMap);			SAMPLER(sampler_CookieMap);
#endif
...
half3 SampleRectangleLightingCookie(float3 positionWS, float3 normalWS)
{
	...
	half  NdotL = saturate(dot(dir, normalOS));
	half  NdotL2 = saturate(dot(-localForward, normalOS) * 0.7h + 0.3h);
	...
	half dist = proj < 0.0h ? 0.0h : 1.0h - saturate(proj * rcp(_RectLightLocalSize.z));
	half dist2Reduce = dist * dist;
	half3 decalColor = SAMPLE_TEXTURE2D(_CookieMap, sampler_CookieMap, ndcHalfPos.xy * 0.5 + 0.5).rgb * _Intensity * NdotL * NdotL2 * dist2Reduce;
