...
public static class OVRPlugin
{
    ...
    public static bool gestureTest = true;

    [System.Serializable]
    public class HandData
    {
        public Hand handType;
        public List<HandStateInternal> handList;

        public HandData()
        {
            handList = new List<HandStateInternal>();
        }
    }


    private static HandData _lHandData;
    private static HandData _rHandData;
    public static int lHandIndex;
    public static int rHandIndex;

    private static Result ReadHandData(Hand hand, out HandStateInternal HandState)
    {
        string path = $"{Application.dataPath}/hand_{hand.ToString()}.json";
        ...
        if (System.IO.File.Exists(path))
        {
            switch (hand)
            {
                case Hand.HandLeft:
                    if (_lHandData == null)
                        _lHandData = JsonUtility.FromJson<HandData>(System.IO.File.ReadAllText(path));

                    if (lHandIndex < _lHandData.handList.Count)
                    {
                        HandState = _lHandData.handList[lHandIndex];
                        lHandIndex++;
                    ...
                    break;
                case Hand.HandRight:
                    _rHandData = JsonUtility.FromJson<HandData>(System.IO.File.ReadAllText(path));

                    if (rHandIndex < _rHandData.handList.Count)
                    {
                        HandState = _rHandData.handList[rHandIndex];
                        // Debug.Log($"{rHandIndex}\n,{ JsonUtility.ToJson(HandState)}");
                        rHandIndex++;
                        return Result.Success;
                    }
                    else
        ...
        HandState = new HandStateInternal();
        return Result.Failure;
    }

    private static void WriteHandData(Hand hand, HandStateInternal HandState)
    {
        if (_lHandData == null) _lHandData = new HandData();
        if (_rHandData == null) _rHandData = new HandData();
        _lHandData.handType = OVRPlugin.Hand.HandLeft;
        _rHandData.handType = OVRPlugin.Hand.HandRight;
        string path = $"{Application.persistentDataPath}/hand_{hand.ToString()}.json";
        switch (hand)
        {
            case Hand.HandLeft:
                _lHandData.handList.Add(HandState);
                if (_lHandData.handList.Count > 500)
                {
                    System.IO.File.WriteAllText(path, JsonUtility.ToJson(_lHandData));
                    _lHandData.handList.Clear();
                }

                break;
            case Hand.HandRight:
                _rHandData.handList.Add(HandState);
                if (_rHandData.handList.Count > 500)
                {
                    System.IO.File.WriteAllText(path, JsonUtility.ToJson(_rHandData));
                    _rHandData.handList.Clear();
                }
		...
		if (version >= OVRP_1_44_0.version)
		{
			...
			if (gestureTest)
            {
                if (Application.platform != RuntimePlatform.Android)
                    res = ReadHandData(hand, out cachedHandState);
                else if (res == Result.Success)
                    WriteHandData(hand, cachedHandState);
            }
    ...
    {
        public SkeletonType handType;
        public List<Skeleton2Internal> skeletonList;

        public SkeletonData()
        {
            skeletonList = new List<Skeleton2Internal>();
        }
    }

    public static SkeletonData _lSkeletonData;
    public static SkeletonData _rSkeletonData;
    public static int _lSkeletonIndex;
    public static int _rSkeletonIndex;


    private static Result ReadSkeletonData(SkeletonType hand, out Skeleton2Internal skeletonState)
    {
        string path = $"{Application.dataPath}/skeleton_{hand.ToString()}.json";
        ...
        if (System.IO.File.Exists(path))
        {
            switch (hand)
            {
                case SkeletonType.HandLeft:
                    if (_lSkeletonData == null)
                        _lSkeletonData = JsonUtility.FromJson<SkeletonData>(System.IO.File.ReadAllText(path));

                    if (_lSkeletonIndex < _lSkeletonData.skeletonList.Count)
                    {
                        skeletonState = _lSkeletonData.skeletonList[_lSkeletonIndex];
                        _lSkeletonIndex++;
                        return Result.Success;
                    }
                    else
                ...
                case SkeletonType.HandRight:
                    _rSkeletonData = JsonUtility.FromJson<SkeletonData>(System.IO.File.ReadAllText(path));

                    if (_rSkeletonIndex < _rSkeletonData.skeletonList.Count)
                    {
                        skeletonState = _rSkeletonData.skeletonList[_rSkeletonIndex];
                        _rSkeletonIndex++;
                        return Result.Success;
                    }
                    else
        ...
        Debug.Log($"{hand},{_lSkeletonIndex}");
        skeletonState = new Skeleton2Internal();
        return Result.Failure;
    }

    private static void WriteSkeletonData(SkeletonType hand, Skeleton2Internal skeletonState)
    {
        if (_lSkeletonData == null) _lSkeletonData = new SkeletonData();
        if (_rSkeletonData == null) _rSkeletonData = new SkeletonData();

        _lSkeletonData.handType = OVRPlugin.SkeletonType.HandLeft;
        _rSkeletonData.handType = OVRPlugin.SkeletonType.HandRight;
        string path = $"{Application.persistentDataPath}/skeleton_{hand.ToString()}.json";
        Debug.Log(path);
        switch (hand)
        {
            case SkeletonType.HandLeft:
                _lSkeletonData.skeletonList.Add(skeletonState);
                if (_lSkeletonData.skeletonList.Count > 0)
                {
                    System.IO.File.WriteAllText(path, JsonUtility.ToJson(_lSkeletonData));
                    _lSkeletonData.skeletonList.Clear();
                }

                break;
            case SkeletonType.HandRight:
                _rSkeletonData.skeletonList.Add(skeletonState);
                if (_rSkeletonData.skeletonList.Count > 0)
                {
                    System.IO.File.WriteAllText(path, JsonUtility.ToJson(_rSkeletonData));
                    _rSkeletonData.skeletonList.Clear();
                }
    ...
    public static bool GetSkeleton2(SkeletonType skeletonType, ref Skeleton2 skeleton)
    {
		...
		if (version >= OVRP_1_55_0.version)
		{
            ...
            {
                if (Application.platform != RuntimePlatform.Android)
                    res = ReadSkeletonData(skeletonType, out cachedSkeleton2);
                else if (res == Result.Success)
                    WriteSkeletonData(skeletonType, cachedSkeleton2);
            }
    ...
    public static MeshData _lMeshData;
    public static MeshData _rMeshData;
    public static int _lMeshIndex;
    public static int _rMeshIndex;


    private static Result ReadMeshData(MeshType hand, out Mesh meshState)
    {
        string path = $"{Application.dataPath}/mesh_{hand.ToString()}.json";
        ...
        if (System.IO.File.Exists(path))
        {
            switch (hand)
            {
                case MeshType.HandLeft:
                    if (_lMeshData == null)
                        _lMeshData = JsonUtility.FromJson<MeshData>(System.IO.File.ReadAllText(path));

                    if (_lMeshIndex < _lMeshData.meshList.Count)
                    {
                        meshState = _lMeshData.meshList[_lMeshIndex];
                        _lMeshIndex++;
                        return Result.Success;
                    }
                    else
                ...
                case MeshType.HandRight:
                    _rMeshData = JsonUtility.FromJson<MeshData>(System.IO.File.ReadAllText(path));

                    if (_rMeshIndex < _rMeshData.meshList.Count)
                    {
                        meshState = _rMeshData.meshList[_rMeshIndex];
                        _rMeshIndex++;
                        return Result.Success;
                    }
                    else
    ...
    private static void WriteMeshData(MeshType hand, Mesh meshState)
    {
        if (_lMeshData == null) _lMeshData = new MeshData();
        if (_rMeshData == null) _rMeshData = new MeshData();

        _lMeshData.handType = OVRPlugin.MeshType.HandLeft;
        _rMeshData.handType = OVRPlugin.MeshType.HandRight;
        string path = $"{Application.persistentDataPath}/mesh_{hand.ToString()}.json";
        switch (hand)
        {
            case MeshType.HandLeft:
                _lMeshData.meshList.Add(meshState);
                if (_lMeshData.meshList.Count > 0)
                {
                    System.IO.File.WriteAllText(path, JsonUtility.ToJson(_lMeshData));
                    _lMeshData.meshList.Clear();
                }
                break;
            case MeshType.HandRight:
                _rMeshData.meshList.Add(meshState);
                if (_rMeshData.meshList.Count > 0)
                {
                    System.IO.File.WriteAllText(path, JsonUtility.ToJson(_rMeshData));
                    _rMeshData.meshList.Clear();
                }
    ...
    public static bool GetMesh(MeshType meshType, out Mesh mesh)
    {
		...
		if (version >= OVRP_1_44_0.version)
		{
            ...
            {
                if (Application.platform != RuntimePlatform.Android)
                    result = ReadMeshData(meshType, out mesh);
                else if (result == Result.Success)
                    WriteMeshData(meshType, mesh);
            }
